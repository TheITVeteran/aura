...
class EnhancedDeviceScanner:
    ...
    async def comprehensive_scan(self) -> list[dict[str, Any]]:
        ...
        # Method 1 & 2: ARP scan and Ping sweep path in parallel
        results = await asyncio.gather(
            self._arp_scan(),
            self._ping_sweep()
        )
        
        devices.extend(results[0]) # ARP
        existing_ips = {d['ip'] for d in devices}
        
        for ping_dev in results[1]: # Ping
        ...
        for device in devices:
            await self._scan_ports_async(device)
    ...
    async def _arp_scan(self) -> list[dict[str, Any]]:
        ...
        try:
            if self.system in ["Linux", "Darwin", "Windows"]:
    ...
    async def _scan_ports_async(self, device: dict):
        ...
        common_ports = {
            ...
            5900: "VNC", 445: "SMB", 139: "NetBIOS", 21: "FTP",
            23: "Telnet", 3306: "MySQL", 5432: "PostgreSQL",
            6379: "Redis", 27017: "MongoDB"
        }
        
        device["open_ports"] = {}
        
        async def _check_port(port, service):
            if await self._is_port_open_async(device["ip"], port):
                return port, service
            return None
            
        tasks = [_check_port(p, s) for p, s in common_ports.items()]
        results = await asyncio.gather(*tasks)
