...
async def test_mlx_client_retries():
    ...
    with patch.object(MLXLocalClient, "_generate_text_async_inner") as mock_inner:
        ...
        if not (result == "Success!"): raise RuntimeError('Assertion failed')
        if not (mock_inner.call_count == 3): raise RuntimeError('Assertion failed')
...
async def test_response_phase_watchdog():
    ...
    with patch("asyncio.wait_for", side_effect=asyncio.TimeoutError):
        ...
        if not (result_state.transition_cause == "generation_timeout"): raise RuntimeError('Assertion failed')
        if not ("timed out" in result_state.cognition.working_memory[-1]["content"]): raise RuntimeError('Assertion failed')
...
async def test_affect_telemetry_sync():
    ...
    if not (ls.update.called): raise RuntimeError('Assertion failed')
    args, kwargs = ls.update.call_args
    logger.info(f"LS Update Args: {kwargs}")
    if not ("valence" in kwargs): raise RuntimeError('Assertion failed')
    if not ("arousal" in kwargs): raise RuntimeError('Assertion failed')
...
async def test_robust_lock():
    ...
    if not (await lock.acquire_robust(timeout=1.0) is True): raise RuntimeError('Assertion failed')
    lock.release()
    ...
    if not (all(results)): raise RuntimeError('Assertion failed')
...
async def main():
    ...
    except (AssertionError, AttributeError, ImportError, OSError, RuntimeError, TimeoutError, TypeError, ValueError) as e:
