...
PROBE_LOG=$SC/verify_probe_$TS.log
LAUNCH=$SC/verify_launch_$TS.log
DONE=$SC/VERIFY_DONE_$TS
cd "$WT" || exit 1
...
sleep 180                       # warmup settle
echo "phase=idle_window minutes=25 t=$(date +%H:%M:%S)"
for i in $(seq 1 50); do        # 50 x 30s = 25 min
...
caffeinate -dims nice -n 10 .venv/bin/python tools/conversation_endurance_probe.py \
  --base http://127.0.0.1:8001 --turns 60 --deadline-min 55 \
  --session "verify-fixset-$TS" > "$PROBE_LOG" 2>&1
PROBE_RC=$?
