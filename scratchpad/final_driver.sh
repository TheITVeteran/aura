...
PROBE_LOG=$SC/final_probe_$TS.log
LAUNCH=$SC/final_launch_$TS.log
DONE=$SC/FINAL_DONE_$TS
cd "$WT" || exit 1
...
echo "phase=idle_window minutes=12 t=$(date +%H:%M:%S)"
for i in $(seq 1 24); do        # 50 x 30s = 25 min
...
caffeinate -dims nice -n 10 .venv/bin/python tools/conversation_endurance_probe.py \
  --base http://127.0.0.1:8001 --turns 30 --deadline-min 40 \
  --session "final-fixset-$TS" > "$PROBE_LOG" 2>&1
PROBE_RC=$?
