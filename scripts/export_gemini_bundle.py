...
def get_file_summary(filepath):
    try:
        ...
        return f"Not summarizable ({type(exc).__name__}: {exc})"

def build_gemini_bundle():
    root_dir = Path(os.environ.get("AURA_SOURCE_DIR", Path(__file__).resolve().parents[1])).expanduser().resolve()
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        for file in files:
            ...
            try:
                ...
                addition = f"\n--- ERROR READING FILE: {rel} ({type(e).__name__}: {e}) ---\n"
                if current_chars + len(addition) < char_limit:
                    bundle_content += addition
                    current_chars += len(addition)
