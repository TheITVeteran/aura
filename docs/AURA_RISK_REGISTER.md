...
| R-029 | Vector memory JSON dumps exhaust RAM and pollute clone/search | High | Medium | Local vector fallback moved to SQLite `float32` BLOB storage; tracked `memory_store/long_term.json` removed; migration script added | Evaluate sqlite-vec virtual tables for ANN acceleration once the extension is packaged locally |
| R-030 | Resource pressure does not throttle swarm debate width | High | Medium | AgentDelegator now derives effective parallelism from IntegrityMonitor CPU/RAM/thermal telemetry and returns deterministic consensus when LLM synthesis is unavailable | Feed richer metabolic budgets into the same throttle and add soak evidence under sustained load |
| R-031 | Will fail-closed closure can become systemic catatonia | Critical | Medium | UnifiedWill now has a signed, reserved self-repair/stabilization relief lane under refusal storms; it cannot authorize tools, memory writes, external actions, or identity violations | Wire daily SelfAudit/ASA repair jobs to request this lane explicitly and prove recovery drills |
