...
class ScreenObserver:
    ...
    async def get_status(self) -> dict[str, Any]:
        ...
        The two *_active checks are coroutines. Unawaited, read_vision()/read_audio() returned
        ...
        In this copy read_vision/read_audio are synchronous, so only the
        two *_active coroutines needed awaiting — but those were the fields
        that made a stopped sensor report active, which is the half that
        misleads. A status that reports a dead sensor as active is worse
        than no status.
        """
        # read_vision/read_audio are synchronous in this copy; only the
        # *_active checks are coroutines.
