...
class HephaestusEngine(AuraBaseModule):
    ...
    async def synthesize_skill(self, capability_name: str, objective: str) -> dict[str, Any]:
        ...
        async def _synthesize_wrapped():
            ...
            logic_body = textwrap.dedent(str(logic_data["code"])).strip("\n")
            if not logic_body.strip():
                logic_body = 'return {"ok": False, "error": "Generated skill body was empty."}'
            code = SKILL_TEMPLATE.format(
                ...
                logic_code=textwrap.indent(logic_body, "                ")
            )
            ...
            try:
                ast.parse(code, filename=str(skill_file))
            ...
            # 6. Save to Disk
            await asyncio.to_thread(skill_file.write_text, code, encoding="utf-8")
