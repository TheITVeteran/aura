...
def _record_state_machine_degradation(
    error: BaseException,
    *,
    action: str,
...
class StateMachine:
    ...
    def _gather_cognitive_context(self, user_input: str) -> str:
        ...
        try:
            ...
            if affect and hasattr(affect, "get_state_sync"):
                ...
                state = {
                    ...
                    "dominance": getattr(affect, "dominance", 0.5),
                }
            else:
                state = None
            ...
            _record_state_machine_degradation(
                e,
                action="continued chat prompt assembly without affect tone block",
            )
        ...
        try:
            ...
            _record_state_machine_degradation(
                e,
                action="continued chat prompt assembly without retrieved memory block",
            )
            capture_and_log(e, {"module": __name__})
        ...
        try:
            ...
            _record_state_machine_degradation(
                e,
                action="continued chat prompt assembly without attention focus block",
            )
            capture_and_log(e, {"module": __name__})
    ...
    async def execute(
        ...
        if intent == Intent.CHAT or intent == Intent.UNKNOWN:
            ...
            if self._should_escalate_to_skill(user_input):
                ...
                response, tools = await self._handle_skill(
                    user_input, context, priority=priority, origin=origin
                )
    ...
    async def _handle_chat(
        ...
        try:
            ...
            try:
                ...
                _record_state_machine_degradation(
                    exc,
                    action="continued chat response without runtime state contract context",
                )
            ...
            if runtime_state is not None:
                ...
                try:
                    ...
                    if is_user_facing_origin(origin) and governed_contract.requires_search:
                        grounded_search_reply = (
                            UnitaryResponsePhase._build_cached_grounded_search_reply(
                                runtime_state,
                                user_input,
                                governed_contract,
                            )
                        ...
                        if (
                            not grounded_search_reply
                            and not governed_contract.tool_evidence_available
                        ):
                            grounded_search_reply = (
                                await UnitaryResponsePhase._attempt_grounded_search_reply(
                                    user_input,
                                    governed_contract,
                                    origin=origin,
                                )
                        ...
                        if grounded_search_reply:
                            ...
                            self._emit_telemetry(
                                {"type": "chat_stream_chunk", "chunk": grounded_search_reply}
                            )
                            self._emit_telemetry({"type": "chat_stream_end"})
                            return grounded_search_reply

                    if is_user_facing_origin(
                        origin
                    ) and UnitaryResponsePhase._should_direct_answer_live_voice(
                        user_input,
                        governed_contract,
                        is_user_facing=True,
                    ):
                        ...
                        if direct_reply:
                            direct_reply, validation = (
                                UnitaryResponsePhase._select_valid_recovery_variant(
                                    direct_reply,
                                    governed_contract,
                                )
                            )
                            if not validation.ok:
                                direct_reply, validation = (
                                    UnitaryResponsePhase._select_valid_recovery_variant(
                                        UnitaryResponsePhase._build_minimal_live_voice_reply(
                                            runtime_state
                                        ),
                                        governed_contract,
                                    )
                            ...
                            if validation.ok:
                                self._emit_telemetry({"type": "chat_stream_start"})
                                self._emit_telemetry(
                                    {"type": "chat_stream_chunk", "chunk": direct_reply}
                                )
                                self._emit_telemetry({"type": "chat_stream_end"})
                                return direct_reply
                    ...
                    _record_state_machine_degradation(
                        exc,
                        action="continued with standard chat path after direct recovery path failed",
                    )
            ...
            while attempt <= max_retries:
                try:
                    ...
                    # to use more of the 32K token buffer on M5 hardware.
                        ...
                        async def stream_and_ui(
                            ...
                            async for event in self.llm.generate_stream(
                                ...
                                if event.type == "thought":
                                    self._emit_telemetry(
                                        {"type": "chat_thought_chunk", "chunk": event.content}
                                    )
                                    continue
                                ...
                                if event.type == "token":
                                    ...
                                    if first_chunk:
                                        ...
                                        if not preview:
                                            continue
                                        preview_validation = validate_dialogue_response(
                                            preview, contract
                                        )
                                        ...
                                        if len(preview) < 120 and not any(
                                            mark in preview for mark in ".!?\n:"
                                        ):
                                            continue
                                        chunk = preview
                                        first_chunk_buffer = ""
                                    ...
                                    if chunk:
                                        ...
                                        self._emit_telemetry(
                                            {"type": "chat_stream_chunk", "chunk": chunk}
                                        )
                                        yield chunk

                                if event.type == "error":
                                    logger.error(
                                        "Streaming error in StateMachine: %s", event.content
                                    )
                            ...
                            if first_chunk and first_chunk_buffer.strip():
                                ...
                                self._emit_telemetry(
                                    {"type": "chat_stream_chunk", "chunk": preview}
                                )
                                yield preview
                        ...
                        if voice_engine and hasattr(voice_engine, "speak_stream"):
                            ...
                            async def _speak_task(
                                ...
                                try:
                                    ...
                                    _record_state_machine_degradation(
                                        e,
                                        action="continued chat stream after background TTS failed",
                                    )
                    ...
                    if attempt < max_retries and any(
                        p in response_preamble for p in banned_phrases
                    ):
                        logger.warning(
                            "🚫 Identity Regression Detected (Attempt %d). Re-aligning...",
                            attempt + 1,
                        )
                        attempt += 1
                    ...
                    logger.warning(
                        "🚫 Generic stream preamble detected (Attempt %d). Re-aligning...",
                        attempt + 1,
                    )
                    ...
                    _record_state_machine_degradation(
                        e,
                        action="retried chat generation or returned degraded chat fallback",
                        severity="error",
                    )
            ...
            if not response or not response.strip():
                logger.warning(
                    "⚠️ _handle_chat: All LLM attempts produced empty response — using fallback"
                )
            ...
            try:
                ...
                _record_state_machine_degradation(
                    tts_err,
                    action="continued chat response after non-critical TTS scheduling failed",
                )
    ...
    async def _handle_skill(
        ...
        skills = (
            self.orchestrator.capability_engine.skills
            if self.orchestrator.capability_engine
            else {}
        )
        active_names = (
            self.orchestrator.capability_engine.active_skills
            if self.orchestrator.capability_engine
            else set()
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    if hasattr(meta, "extract_and_validate_args"):
                        validated_params = await meta.extract_and_validate_args(
                            json.dumps(params), self.llm
                        )
                except (json.JSONDecodeError, TypeError, ValueError) as eval_err:
                    _record_state_machine_degradation(
                        eval_err,
                        action="executed skill with raw parameters after validation failed",
                    )
                ...
                if match:
                    ...
                    try:
                        ...
                        return await self._execute_skill_logic(
                            tool_name, params, user_input, priority=priority, origin=origin
                        )
                    except (json.JSONDecodeError, TypeError, ValueError) as exc:
                        _record_state_machine_degradation(
                            exc,
                            action="executed skill with empty fallback parameters",
                        )
                        logger.debug(
                            "Fallback skill params parse failed for %s: %s", tool_name, exc
                        )
                        return await self._execute_skill_logic(
                            tool_name, {}, user_input, priority=priority, origin=origin
                        )
            ...
            _record_state_machine_degradation(
                e,
                action="returned skill-routing failure response and emitted error state",
                severity="error",
            )
            logger.error("Skill routing failed: %s", e, exc_info=True)
            self._emit("State: ERROR", f"Skill routing exception: {str(e)[:50]}")
            return (
                f"Skill routing failed before a coherent action could be selected: {str(e)[:80]}",
                [],
            )
    ...
    def _infer_live_artifact_path(self, user_input: str) -> str:
        ...
        suffix = (
            "html" if any(t in text.lower() for t in ("game", "html", "browser", "snake")) else "py"
        )
...
"""
    ...
    async def _maybe_execute_live_coding_artifact(
        ...
        *,
        ...
        if not code.strip():
            return (
                "The coding path did not produce a runnable artifact, so I refused to write an empty file.",
                [],
            )
        ...
        if not (isinstance(write_result, dict) and write_result.get("ok")):
            detail = (
                write_result.get("error") if isinstance(write_result, dict) else str(write_result)
            )
            return f"I generated the artifact but the governed file write failed: {detail}", [
                "coding_skill"
            ]
        ...
        try:
            ...
            if autonomic and isinstance(result, dict) and result.get("message"):
                ...
                if framed_observation:
                    ...
                    source_context = (
                        f"Source: {result.get('source', result.get('url', 'Unknown'))}\n"
                    )
                else:
                    content_to_summarize = str(result)
                    source_context = ""

                content_snippet = str(content_to_summarize)[
                    :5000
                ]  # Increased to 5k for deep research
                summary_prompt = (
                ...
                try:
                    ...
                    _record_state_machine_degradation(
                        e,
                        action="returned skill result/message fallback after summary failed",
                    )
            ...
            _record_state_machine_degradation(
                e,
                action="returned skill execution error response and emitted error state",
                severity="error",
            )
    ...
    def _emit(self, status: str, detail: str):
        ...
        try:
            get_event_bus().publish_threadsafe(
                "status_update", {"component": status, "status": detail}
            )
        except (ImportError, AttributeError, RuntimeError) as e:
            _record_state_machine_degradation(
                e,
                action="continued after non-critical status event emission failed",
            )
