...
class RayHit:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"body_id": self.body_id, "distance": round(self.distance, 6),
...
class EmbodiedAgent:
    ...
    def __init__(self, world: PhysicsWorld, blueprint: WorldBlueprint | None,
                 agent_id: str = "agent"):
        self.world = world
    ...
    @classmethod
    def spawn(cls, world: PhysicsWorld, blueprint: WorldBlueprint | None,
              *, agent_id: str = "agent",
              position: tuple[float, float, float] | None = None) -> "EmbodiedAgent":
    ...
    def grounded(self) -> bool:
        ...
        hit = self.raycast(direction=(0.0, 0.0, -1.0),
                           max_distance=AGENT_RADIUS + _GROUND_EPSILON)
    ...
    def raycast(self, *, direction: tuple[float, float, float],
                max_distance: float = 50.0,
                origin: tuple[float, float, float] | None = None) -> RayHit | None:
        ...
        o = (np.asarray(origin, dtype=np.float64)
             if origin is not None else self.body.position.copy())
    ...
    def walk(self, *, heading: float | None = None, ticks: int = 1) -> None:
        ...
        for _ in range(max(1, int(ticks))):
            ...
            drive = np.array([
                math.cos(self.state.yaw), math.sin(self.state.yaw), 0.0
            ]) * accel * self.world.dt
    ...
    def throw(self, *, speed: float = 6.0, pitch: float = 0.35) -> str:
        ...
        speed = max(0.0, min(float(speed), 25.0))
        held = self.world.body(self.state.held_body)
        held.velocity = np.array([
    ...
    def _carry_held_body(self) -> None:
        ...
        held = self.world.body(self.state.held_body)
        offset = np.array([
            math.cos(self.state.yaw), math.sin(self.state.yaw), 0.4
    ...
    def navigate_to(self, target_xy: tuple[float, float],
                    *, tolerance: float = 1.0,
                    max_ticks: int = _MAX_NAV_TICKS) -> dict[str, Any]:
        """Plan and execute a walk to ``target_xy``. Reports the outcome
        honestly: reached / no_path / timed_out, with the distance left."""
        max_ticks = max(1, min(int(max_ticks), _MAX_NAV_TICKS))
        ...
        while ticks_used < max_ticks:
            ...
            if float(np.linalg.norm(position - target)) <= tolerance:
                result = {
                    "status": "reached", "target": list(target_xy),
                    "ticks_used": ticks_used,
                    "final_distance": round(float(np.linalg.norm(position - target)), 4),
                }
                self.state.last_navigation = result
                return result
            while waypoint_index < len(path) - 1 and float(np.linalg.norm(
                position - np.array(path[waypoint_index])
            )) < _WAYPOINT_RADIUS:
                waypoint_index += 1
            ...
            ticks_used += 4
        result = {
            "status": "timed_out", "target": list(target_xy),
            "ticks_used": ticks_used,
            "final_distance": round(
                float(np.linalg.norm(self.body.position[:2] - target)), 4),
        }
        self.state.last_navigation = result
        return result
