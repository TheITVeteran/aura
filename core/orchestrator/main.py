...
_ORCHESTRATOR_RECOVERABLE_ERRORS = (
    AttributeError,
    ImportError,
    LookupError,
    OSError,
...
class RobustOrchestrator(
    ...
    OrchestratorServicesMixin,
    OutputFormatterMixin,
    PersonalityBridgeMixin,
    CognitiveBackgroundMixin,
    MessagePipelineMixin,
    ToolExecutionMixin,
    LearningEvolutionMixin,
    AutonomyMixin,
    ResponseProcessingMixin,
    ContextStreamingMixin,
            ...
            if self._actor_bus is None:
                ...
                try:
                    ...
                    _record_main_degradation(
                        ...
                        action="continued with local ActorBus after container registration failed",
                    )
    ...
    def _publish_status(self, data: dict[str, Any]):
        ...
        try:
            ...
            get_event_bus().publish_threadsafe(
                "status", {"timestamp": time.time(), "status": self.status.model_dump(), **data}
            )
        except _ORCHESTRATOR_RECOVERABLE_ERRORS as exc:
            _record_main_degradation(
                exc,
                action="dropped status event-bus update and kept orchestrator status in memory",
            )
    ...
    def _background_message_block_reason(self, origin: str) -> str:
        ...
        except _ORCHESTRATOR_RECOVERABLE_ERRORS as _exc:
            _record_main_degradation(
                _exc,
                action="continued background admission check without foreground quiet-window signal",
            )
        ...
        try:
            ...
            _record_main_degradation(
                _exc,
                action="continued background admission check without LLM pressure signal",
            )
        ...
        try:
            ...
            _record_main_degradation(
                _exc,
                action="continued background admission check without InferenceGate deferral signal",
            )
        ...
        try:
            ...
            _record_main_degradation(
                _exc,
                action="allowed background admission check to continue without psutil memory reading",
            )
    ...
    def __init__(
        ...
        self.auto_fix_enabled = (
            auto_fix_enabled if auto_fix_enabled is not None else config.security.auto_fix_enabled
        )
    ...
    async def start(self):
        ...
        try:
            ...
            if not v_result.passed:
                ...
                logger.warning(
                    "⚠️ Boot pre-check: %d issues (non-fatal): %s",
                    len(v_result.failures),
                    v_result.failures,
                )
            ...
            _integrity_msg = (
                "Validated"
                if not self._boot_warnings
                else f"Validated with {len(self._boot_warnings)} warnings"
            )
            logger.info("══════════════════════════════════════════")
            ...
            _record_main_degradation(
                e,
                action="failed orchestrator boot pre-check and returned startup failure",
                severity="error",
            )
        ...
        try:
            ...
            if lightweight_test_boot:
                logger.info(
                    "🧪 Lightweight pytest runtime boot: skipping continuous background services."
                )
                return True
            ...
            if _foreground_only_runtime() or not _env_flag("AURA_ENABLE_PROACTIVE_VISION", True):
                ...
                try:
                    ...
                    _record_main_degradation(
                        e,
                        action="continued startup without continuous sensory buffer",
                    )
            ...
            try:
                ...
                _record_main_degradation(
                    e,
                    action="continued startup without persisted continuity session count",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _we,
                    action="continued startup after waking-sequence emission failed",
                )
            ...
            if self.self_model:
                try:
                    ...
                    from core.runtime.errors import StateCoherenceFailure

                    _record_main_degradation(
                        StateCoherenceFailure(f"Failed to load Self-Model state: {e}"),
                        action="continued startup with in-memory self-model state after persistence load failed",
                        severity="error",
                    )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _ai_err:
                from core.runtime.errors import StateCoherenceFailure

                _record_main_degradation(
                    StateCoherenceFailure(f"Architecture index boot init non-fatal: {_ai_err}"),
                    action="continued startup without architecture self-awareness index registration",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _circ_err:
                _record_main_degradation(
                    _circ_err,
                    action="continued startup without prewarmed affective circumplex",
                )
            ...
            try:
                ...
                logger.info(
                    "♥ HeartstoneValues online: %s",
                    {k: round(v, 2) for k, v in _hsv.values.items()},
                )
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _hsv_err:
                _record_main_degradation(
                    _hsv_err,
                    action="continued startup without registered HeartstoneValues service",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _ef_err:
                _record_main_degradation(
                    _ef_err,
                    action="continued startup without registered epistemic filter",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _st_err:
                _record_main_degradation(
                    _st_err,
                    action="continued startup without autonomous sleep trigger loop",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _pe,
                    action="continued startup without PNEUMA active-inference service",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _mhaf_err,
                    action="continued startup without MHAF consciousness substrate loop",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _ais_err:
                _record_main_degradation(
                    _ais_err,
                    action="continued startup without active-inference sampler",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _neo_err:
                _record_main_degradation(
                    _neo_err,
                    action="continued startup without private neologism engine",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _term_err,
                    action="continued startup without terminal fallback watchdog",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _e:
                _record_main_degradation(
                    _e,
                    action="continued startup without CRSM bidirectional self-model service",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without higher-order thought engine",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without hedonic gradient service",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without counterfactual engine",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without AGI-layer curiosity/planning/synthesis services",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without agency-layer commitment/compute/identity services",
                )
            ...
            try:
                ...
                if not _ur.has_passphrase():
                    logger.warning(
                        "⚠️  No owner passphrase set. Run: python -m core.security.user_recognizer --setup"
                    )
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as _e:
                _record_main_degradation(
                    _e,
                    action="continued startup without security-layer service registration",
                    severity="error",
                )
            ...
            try:
                ...
                if _foreground_only_runtime():
                    logger.info(
                        "ExperienceConsolidator background loop disabled for foreground-only boot."
                    )
                else:
                    self._fire_and_forget(
                        _consolidator.start(), name="orchestrator.experience_consolidator.start"
                    )
                logger.info(
                ...
                _record_main_degradation(
                    _e,
                    action="continued startup without substrate embodiment service registration",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    e,
                    action="continued startup without thawed cognitive snapshot",
                )
            ...
            try:
                ...
                _record_main_degradation(
                    e,
                    action="continued startup without Lazarus brainstem recovery service",
                )
            ...
            if (
                ...
                and hasattr(self, "consciousness")
                ...
                logger.info(
                    "Consciousness/curiosity background loops disabled for foreground-only boot."
                )
            ...
            try:
                ...
                _record_main_degradation(
                    ac_err,
                    action="continued startup without AgencyCore registration",
                    severity="error",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as sa_err:
                _record_main_degradation(
                    sa_err,
                    action="continued startup without SubsystemAudit heartbeat service",
                )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as im_err:
                _record_main_degradation(
                    im_err,
                    action="continued startup without system integrity monitor",
                )
            ...
            try:
                ...
                if existing_monitor is not None:
                    ...
                    self._event_loop_monitor = EventLoopMonitor(
                        threshold=loop_lag_threshold, interval=1.0
                    )
                    self._event_loop_monitor.start()
                    logger.info("✓ Event Loop Monitor active")
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as el_err:
                _record_main_degradation(
                    el_err,
                    action="continued startup without event-loop lag monitor",
                )
            ...
            _record_main_degradation(
                e,
                action="failed orchestrator startup and reset running flag",
                severity="error",
            )
    ...
    async def run(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                _record_main_degradation(
                    e,
                    action="kept heartbeat loop alive after cycle update failed",
                    severity="error",
                )
    ...
    async def _register_dream_tasks(self, scheduler):
        ...
        await scheduler.register(
            TaskSpec(
                ...
                coro=lambda: coordinator.run_if_due(
                    "biological_sleep", _run_dreamerv2, 7200, priority=2
                ),
    ...
    async def _register_scheduled_tasks(self):
        ...
        if self.metabolic_coordinator:
            ...
            if hasattr(self.metabolic_coordinator, "process_cycle"):
                ...
                logger.warning(
                    "⚠️ metabolic_coordinator (%s) lacks 'process_cycle'. Skipping registration.",
                    type(self.metabolic_coordinator).__name__,
                )
        else:
        ...
        async def meta_evolution_wrapper():
            if hasattr(self, "meta_cognition") and self.meta_cognition:
                ...
                try:
                    ...
                    if mem.percent > 75:
                        logger.debug(
                            "🌀 Meta-Evolution skipped: memory pressure %.1f%%", mem.percent
                        )
                        return
                except _ORCHESTRATOR_RECOVERABLE_ERRORS as _exc:
                    _record_main_degradation(
                        _exc,
                        action="continued meta-evolution scheduling without memory-pressure reading",
                    )
                ...
                try:
                    ...
                    _record_main_degradation(
                        exc,
                        action="skipped current meta-evolution cycle after evolution step failed",
                    )
    ...
    async def _handle_periodic_tasks(self):
        ...
        """
        pulse = self._pulse_subsystem_audit()
        if inspect.isawaitable(pulse):
            await pulse
    ...
    def _pulse_subsystem_audit(self):
        ...
        try:
            ...
            if (
                (hasattr(self, "substrate") and self.substrate)
                or (hasattr(self, "liquid_substrate") and self.liquid_substrate)
                or getattr(self, "_liquid_substrate", None)
            ):
                audit.heartbeat("liquid_substrate")

            # 4. Drive Controller
            if (
                (hasattr(self, "drive_controller") and self.drive_controller)
                or (hasattr(self, "drives") and self.drives)
                or getattr(self, "_drive_controller", None)
            ):
            ...
            if (
                ...
                or getattr(self, "_self_model", None)
            ):
            ...
            if (
                (hasattr(self, "scanner") and self.scanner)
                or (hasattr(self, "sovereign_scanner") and self.sovereign_scanner)
                or getattr(self, "_sovereign_scanner", None)
            ):
            ...
            _record_main_degradation(
                e,
                action="skipped subsystem health pulse after audit heartbeat emission failed",
            )
    ...
    def _fire_and_forget(self, coro, name: str | None = None):
        ...
        if not (asyncio.iscoroutine(coro) or inspect.iscoroutine(coro)):
            ...
            if inspect.iscoroutinefunction(coro):
                logger.warning(
                    "Passed coroutine function instead of coroutine to fire_and_forget: %s", coro
                )
                return None
    ...
    async def _recover_from_stall(self):
        ...
        except _ORCHESTRATOR_RECOVERABLE_ERRORS as dlq_e:
            _record_main_degradation(
                dlq_e,
                action="continued stall recovery after dead-letter capture failed",
                severity="error",
            )
        ...
        try:
            ...
            if self.message_queue.qsize() > 50:
                ...
                if dropped:
                    try:
                        ...
                        _record_main_degradation(
                            e,
                            action="continued stall recovery after dropped-message DLQ file write failed",
                            severity="error",
                        )
            ...
            except _ORCHESTRATOR_RECOVERABLE_ERRORS as df_err:
                _record_main_degradation(
                    df_err,
                    action="continued stall recovery after substrate defrag failed",
                    severity="error",
                )
            ...
            if (
                ...
                and origin in ("user", "voice", "admin")
            ):
                ...
                if msg_hash in self._poison_pill_cache:
                    logger.critical(
                        "🚫 [RECOVERY] Poison pill detected (%s). Skipping auto-retry.", msg_hash
                    )
            ...
            if self._recovery_attempts >= 3:
                ...
                logger.critical(
                    "🚨 STALL PERSISTS: Escalating to internal orchestrator restart (Hot-Reload)."
                )
            ...
            _record_main_degradation(
                e,
                action="stopped stall recovery sequence after recovery orchestration failed",
                severity="error",
            )
    ...
    async def process_event(self, event: Any, origin: Any = "internal", priority: int = 20):
        ...
        if isinstance(origin, dict):
            ...
            if priority == 20 and "priority" in payload:
                try:
                    ...
                    logger.debug("Priority override ignored for telemetry event: %s", exc)
        elif origin is None:
        ...
        origin_label = str(normalized_origin or "internal")
        if not self._authorize_background_enqueue_sync(event, origin_label, priority):
            logger.info("🛡️ Legacy event enqueue blocked for %s.", origin_label)
            return False

        try:
            from core.schemas import IPCMessage

            event = self._deep_circular_safe_sanitize(event)
            ...
            logger.warning(
                "⚠️ Message queue full. Dropped legacy event from %s: %s",
                origin_label,
                str(event)[:120],
            )
            return False
    ...
    async def _ensure_inference_gate_ready(self, context: str = "runtime") -> bool:
        ...
        try:
            ...
            if lightweight_test_boot:
                ...
                logger.info(
                    "🧪 Lightweight pytest boot: deferring InferenceGate init during %s.", context
                )
            else:
        ...
        try:
            ...
            try:
                ...
                _record_main_degradation(
                    exc,
                    subsystem="orchestrator",
                    action="continued sensory actor startup with local ActorBus after registration failed",
                )
                logger.debug("Actor bus registration skipped: %s", exc)
            ...
            if self.supervisor:
                ...
                self._actor_bus.register_handler(
                    "SensoryGate", "SENSORY_UPDATE", self._handle_sensory_update
                )
                self._actor_bus.register_handler(
                    "SensoryGate", "COMMIT_STATE", self._handle_remote_commit
                )
                self._actor_bus.register_handler(
                    "SensoryGate", "HEARTBEAT", self._handle_actor_heartbeat
                )
                logger.info("🛡️ SensoryGateActor managed by Supervision Tree.")
            else:
                logger.error(
                    "❌ Cannot start SensoryGateActor: Supervisor Tree not available in container."
                )

        except _ORCHESTRATOR_RECOVERABLE_ERRORS as e:
            _record_main_degradation(
                e,
                action="continued startup without supervised SensoryGate actor",
                severity="error",
            )
    ...
    async def _update_liquid_pacing(self):
        ...
        while not self._stop_event.is_set():
            ...
            if self.status.cycle_count % 10 == 0:
                ...
                async def _optimize_dbs():
                    try:
                        ...
                        _record_main_degradation(
                            e,
                            action="skipped current database vacuum maintenance cycle",
                        )
            ...
            if (
                self.status.cycle_count % 50 == 0
                and len(self.conversation_history) > 10
            ...
            if self.status.cycle_count % 100 == 0:
                ...
                if hasattr(self, "memory") and self.memory:
                    ...
                    try:
                        ...
                        _record_main_degradation(
                            e,
                            action="skipped current vector-memory pruning maintenance cycle",
                        )
    ...
    async def _run_terminal_self_heal(self):
        ...
        try:
                ...
                if error_goal and not is_thinking:
                    try:
                        ...
                        if policy_reason:
                            ...
                            return
                    except _ORCHESTRATOR_RECOVERABLE_ERRORS as policy_exc:
                        _record_main_degradation(
                            policy_exc,
                            action="continued terminal self-heal policy evaluation with conservative allow decision",
                        )
                    ...
                    if sm:
                        sm.on_error(
                            Exception(
                                f"Terminal Command Failure: {error_goal.get('error', 'Unknown')}"
                            )
                            if isinstance(error_goal.get("error"), str)
                            else Exception("Terminal Command Failure"),
                            {
                                "command": error_goal.get("command"),
                                "output": error_goal.get("output"),
                            },
                    ...
                    self._current_thought_task = get_task_tracker().track_task(
                        get_task_tracker().create_task(
                            self.process_user_input_priority(
                                error_goal["objective"], origin="terminal_monitor"
                            )
                        )
                    )
        except _ORCHESTRATOR_RECOVERABLE_ERRORS as e:
            _record_main_degradation(
                e,
                action="skipped terminal self-heal cycle after monitor check failed",
            )
    ...
    async def process_unprompted_stimulus(
        self, modality: str, data: Any, context: str = ""
    ) -> None:
        ...
        try:
            ...
            _record_main_degradation(
                e,
                action="continued sensory stimulus handling after transcript logging failed",
            )
        ...
        try:
            ...
            _record_main_degradation(
                e,
                action="blocked unprompted sensory stimulus because initiative authority failed",
                severity="error",
            )
        ...
        await self.process_user_input_priority(
            f"[ENVIRONMENTAL TRIGGER]: {context}", origin="sensory_motor"
        )
    ...
    async def generate_voice_response(self, user_text: str) -> str:
        ...
        try:
            ...
            _record_main_degradation(
                e,
                action="continued voice response generation after voice input transcript logging failed",
            )
        ...
        if not response:
            response = (
                "The voice turn did not produce a coherent reply; I logged the degraded output."
            )
        ...
        try:
            ...
            _record_main_degradation(
                e,
                action="returned voice response after voice output transcript logging failed",
            )
    ...
    async def _deadlock_watchdog(self):
        ...
        while not _event_is_set(self._stop_event):
            try:
                ...
                _record_main_degradation(
                    e,
                    action="kept deadlock watchdog alive after watchdog recovery check failed",
                    severity="error",
                )
        ...
        try:
            ...
            if bus:
                try:
                    ...
                    logger.debug(
                        "Telemetry actor-bus publish skipped without running loop: %s", exc
                    )
            ...
            _record_main_degradation(
                e,
                action="dropped fast-path telemetry update after actor-bus and thought-stream emission failed",
            )
    ...
    # _original_handle_incoming_logic -> IncomingLogicMixin (mixin reference)
        ...
        for i, call in enumerate(plan.get("tool_calls", [])):
            ...
            if (i + 1) % 3 == 0:
                try:
                    ...
                    _record_main_degradation(
                        e,
                        action="continued plan execution after critic evaluation failed",
                    )
        ...
        while self.status.running:
            try:
                ...
                if message:
                    ...
                    get_task_tracker().track_task(
                        self.process_user_input_priority(message, origin=origin)
                    )
                else:
                ...
                _record_main_degradation(
                    e,
                    action="kept event listener loop alive after user_input event handling failed",
                    severity="error",
                )
...
def create_orchestrator(**kwargs) -> RobustOrchestrator:
    ...
    with _orchestrator_lock:
        ...
        try:
            ...
            _record_main_degradation(
                exc,
                action="created minimal fallback orchestrator after full orchestrator construction failed",
                severity="critical",
            )
