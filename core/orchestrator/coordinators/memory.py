...
class MemoryCoordinator:
    ...
    def memory(self):
        if self._memory is None:
            if getattr(self, "_loading_memory", False):
                logger.warning(
                    "MemoryCoordinator: Circular memory resolution detected! Falling back."
                )
                return None
            ...
            try:
                ...
                if facade is None:
                    logger.warning(
                        "MemoryCoordinator: memory_facade not found! Falling back to vault directly."
                    )
                    ...
                    try:
                        ...
                        _record_memory_degradation(
                            e,
                            action="left memory facade unavailable after vault fallback failed",
                            severity="error",
                        )
                ...
                if facade is self:
                    logger.warning(
                        "MemoryCoordinator: memory_facade resolved to self! Bypassing to main memory system."
                    )
                    from core.memory.black_hole_vault import get_vault
                    ...
                    try:
                        ...
                        _record_memory_degradation(
                            e,
                            action="left memory facade unavailable after self-resolution vault fallback failed",
                            severity="error",
                        )
                        self._memory = None
                else:
    ...
    async def get_hot_memory(self, limit: int = 5) -> dict[str, Any]:
        ...
        if self.memory:
            try:
                ...
                _record_memory_degradation(
                    e,
                    action="returned empty hot-memory context",
                )
    ...
    async def commit_interaction(
        ...
        role: str | None = None,  # Legacy support
        content: str | None = None,  # Legacy support
    ) -> None:
        ...
        if self.memory and hasattr(self.memory, "commit_interaction"):
            try:
                ...
                _record_memory_degradation(
                    e,
                    action="skipped interaction memory write and kept caller flow alive",
                    severity="error",
                )
                logger.error("MemoryCoordinator.commit_interaction failed: %s", e)

    async def log_event(
        self, event_type: str, content: Any, metadata: dict[str, Any] | None = None
    ) -> None:
        """Delegates to commit_interaction for compatibility with MemoryFacade callers."""
        await self.commit_interaction(
            "system_event", f"[{event_type}] {content}", metadata=metadata
        )
        ...
        if self.memory and hasattr(self.memory, "prune_low_salience"):
            try:
                if asyncio.iscoroutinefunction(self.memory.prune_low_salience):
                    ...
                    await asyncio.to_thread(
                        self.memory.prune_low_salience, threshold_days=threshold_days
                    )
            except (RuntimeError, AttributeError, TypeError, ValueError) as e:
                _record_memory_degradation(
                    e,
                    action="skipped pruning and left existing memory intact",
                )
            ...
            if hasattr(self.memory, "get_cold_memory_context"):
                if asyncio.iscoroutinefunction(self.memory.get_cold_memory_context):
                    ...
                    res = await asyncio.to_thread(
                        self.memory.get_cold_memory_context, query, limit=limit
                    )
                return res if isinstance(res, dict) else {"results": res}
        except (RuntimeError, AttributeError, TypeError) as e:
            _record_memory_degradation(
                e,
                action="returned empty retrieval results",
            )
    ...
    async def consolidate_working_memory(self, state: Any, **kwargs) -> None:
        ...
        try:
            ...
            if router:
                ...
                if summary:
                    logger.info(
                        "🌙 [SUBCONSCIOUS] Memory consolidated: %s",
                        summary[:70].replace("\n", " ") + "...",
                    )
                    ...
                    try:
                        ...
                        _record_memory_degradation(
                            _exc,
                            action="continued consolidation without thought-stream event",
                        )
                        logger.debug("Thought-stream consolidation event skipped: %s", _exc)
                    # Commit to long-term memory
                    await self.commit_interaction(
                        "system_insight", f"[Dream Consolidation] {summary}"
                    )

                    # Truncate working memory (keep only the last 2 messages for immediate context)
                    logger.info(
                        "🌙 [SUBCONSCIOUS] Truncating working memory from %d to 2 items.",
                        len(state.cognition.working_memory),
                    )
                    state.cognition.working_memory = state.cognition.working_memory[-2:]
        except (ImportError, AttributeError, RuntimeError) as e:
            _record_memory_degradation(
                e,
                action="left working memory unchanged after consolidation failure",
                severity="error",
            )
