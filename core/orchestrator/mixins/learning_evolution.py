...
class LearningEvolutionMixin:
    ...
    async def _learn_from_exchange(self, user_message: str, aura_response: str):
        ...
        try:
            ...
            if not kg:
                ...
                try:
                    ...
                    _record_learning_degradation(
                        e,
                        action="skipped exchange learning because knowledge graph could not be created",
                    )
            ...
            kg.add_knowledge(
                content=exchange_summary, type="observation", source="conversation", confidence=0.6
            )
            ...
            if self.cognitive_engine:
                try:
                    ...
                    _record_learning_degradation(
                        e,
                        action="kept raw exchange observation but skipped structured knowledge extraction",
                    )
            ...
            for trigger in ["my name is ", "i'm ", "i am ", "call me "]:
                if trigger in lower_msg:
                    ...
                    if name_candidate and len(name_candidate) > 1:
                        kg.remember_person(
                            name_candidate,
                            {"context": (user_message or "")[:200], "timestamp": time.time()},
                        )
                        break
            ...
            _record_learning_degradation(
                e,
                action="skipped exchange learning after conversation learning pipeline failed",
            )
        ...
        try:
            ...
            if memory_engine:
                ...
                valence = (
                    affect.get_current_state().get("valence", 0.0)
                    if affect and hasattr(affect, "get_current_state")
                    else 0.0
                )
                ...
                get_task_tracker().create_task(
                    memory_engine.store(
                        f"User: {user_message} → Aura: {aura_response}",
                        valence=valence,
                        importance=0.7,
                    )
                )
            ...
            _record_learning_degradation(
                e,
                action="kept chat response path live but skipped long-term memory storage",
            )
    ...
    async def _run_self_update(self):
        ...
        try:
            ...
            _record_learning_degradation(
                e,
                action="returned without scheduling autonomous self-update task",
                severity="error",
            )
    ...
    async def _run_meta_evolution(self):
        ...
        try:
            ...
            _record_learning_degradation(
                e,
                action="ended meta-evolution cycle without applying optimization",
                severity="error",
            )
    ...
    async def _peer_generate_and_persist_goal(self):
        ...
        try:
            ...
            thought_context = (
                " ".join([t["thought"] for t in archive[-5:]])
                if archive
                else "Initial sovereign state"
            )
            ...
            if cognition:
                ...
                if goal_text:
                    ...
                    if motivation and hasattr(motivation, "add_goal"):
                        ...
                        logger.info(
                            "🎯 Peer Mode: New autonomous goal persisted: %s", goal_text[:50]
                        )

        except (RuntimeError, AttributeError, TypeError) as e:
            _record_learning_degradation(
                e,
                action="returned without persisting peer-generated goal",
                severity="error",
            )
    ...
    async def _safe_self_modification_loop(self):
        ...
        while getattr(self.status, "running", False):
            try:
                ...
                _record_learning_degradation(
                    exc,
                    action="deferred self-modification loop after runtime-mode check failed",
                    severity="error",
                )
            ...
            try:
                ...
                if not allowed:
                    logger.info(
                        "🛠️ Peer Mode: Self-modification cycle deferred by authority: %s", reason
                    )
                    await asyncio.sleep(3600)
                    continue
                ...
                _record_learning_degradation(
                    exec_err,
                    action="deferred self-modification loop after authority gate failed",
                    severity="error",
                )
                        ...
                        if proposal:
                            ...
                            if agency and hasattr(agency, "review_modification_safety"):
                                if await agency.review_modification_safety(proposal):
                                    ...
                                    logger.info(
                                        "🛠️ Peer Mode: Sovereign self-modification applied safely."
                                    )

            except (RuntimeError, AttributeError, TypeError) as e:
                _record_learning_degradation(
                    e,
                    action="kept self-modification loop alive after safe modification pulse failed",
                )
