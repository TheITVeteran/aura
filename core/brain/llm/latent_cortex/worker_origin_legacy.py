"""Legacy producer-owned worker chains retained for integrity-only continuity.

This protocol can prove that persisted result bytes have not changed. It cannot
prove worker execution origin because the producer process owns the signing key.
New claim-capable campaigns must use the detached-supervisor v4 protocol.
"""
...
LEGACY_WORKER_AUTHORIZATION_PAYLOAD_SCHEMA = (
    "aura.latent_cortex.worker_authorization_payload.v3"
)
LEGACY_WORKER_RESULT_ORIGIN_SCHEMA = (
    "aura.latent_cortex.worker_result_origin.v1"
)
LEGACY_WORKER_RESULT_SIGNED_PAYLOAD_SCHEMA = (
...
def build_legacy_worker_authorization_payload(
    *,
    campaign_name: str,
    policy_sha256: str,
    protocol_sha256: str,
    ...
    """Build the producer-key v3 continuity contract."""

    if not isinstance(worker_public_key_raw, bytes) or len(worker_public_key_raw) != 32:
        _fail("worker_public_key_invalid")
    for role, value in (("campaign_name", campaign_name), ("arm", arm)):
        _identifier(value, role=role)
    _positive_int(worker_attempt_slot, role="worker_attempt_slot")
    ...
    if (
        ...
        _fail("worker_command_invalid")
    return {
        "schema": LEGACY_WORKER_AUTHORIZATION_PAYLOAD_SCHEMA,
...
def verify_legacy_worker_authorization(
    ...
    *,
    ...
    """Verify one v3 authorization without granting claim-proof custody."""
    ...
    try:
        ...
        raise WorkerOriginError(
            "worker_authorization_attestation_invalid"
        ) from exc


def build_legacy_worker_result_origin(
    *,
    ...
    """Sign one integrity-only v2 result using a producer-owned key."""

    authorization = _validated_authorization_payload(authorization_payload)
    _positive_int(sequence, role="worker_result_sequence")
    ...
    signed_payload = {
        "schema": LEGACY_WORKER_RESULT_SIGNED_PAYLOAD_SCHEMA,
    ...
    material = {
        "schema": LEGACY_WORKER_RESULT_ORIGIN_SCHEMA,
...
def verify_legacy_worker_result_origin(
    policy: VerifiedCampaignTrustPolicy,
    *,
    authorization_attestation: Mapping[str, Any],
    ...
    """Verify a v3/v2 chain for byte integrity only."""

    _identifier(expected_cell_id, role="expected_cell_id")
    _identifier(expected_attempt_id, role="expected_attempt_id")
    _positive_int(expected_sequence, role="worker_result_expected_sequence")
    ...
    if origin.get("schema") != LEGACY_WORKER_RESULT_ORIGIN_SCHEMA:
        _fail("worker_result_origin_invalid")
    ...
    if (
        not isinstance(signed_payload, dict)
        or set(signed_payload) != _RESULT_PAYLOAD_KEYS
    ):
    ...
    _positive_int(
        signed_payload.get("sequence"), role="worker_result_sequence"
    )
