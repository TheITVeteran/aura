...
def process_depth_band(transition_index: int, transition_count: int) -> str:
    """Map a local transition to a stable normalized-depth calibration cell."""

    if (
        type(transition_index) is not int
        or type(transition_count) is not int
        or not 1 <= transition_count <= 256
        or not 0 <= transition_index < transition_count
    ):
        raise ValueError("process transition depth is invalid")
    # Use the cell midpoint so short traces do not place their final transition
    # in an earlier band merely because the denominator is small.
    progress = (transition_index + 0.5) / transition_count
    if progress <= 1.0 / 3.0:
        return "early"
    if progress <= 2.0 / 3.0:
        return "middle"
...
def transition_features(
    ...
    return tuple(float(item) for item in np.concatenate((prior, candidate, delta, np.abs(delta))))
...
class MistakeTransitionExample:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "feature_sha256": _sha256([round(value, 10) for value in self.features]),
...
def _auc(probabilities: np.ndarray, labels: np.ndarray) -> float:
    ...
    while start < len(order):
        ...
        while end < len(order) and ordered_probabilities[end] == ordered_probabilities[start]:
            end += 1
...
def _validate_split(
    ...
    *,
    ...
    if len(widths) != 1 or (state_width is not None and widths != {state_width}):
    ...
    if len({rows[0].trace_receipt_sha256 for rows in traces.values()}) != len(traces):
    ...
    for trace_id, rows in traces.items():
        ...
        if (
            ...
            or [row.transition_index for row in rows] != list(range(first.transition_count))
            or any(
                (
                    row.task_id,
                    row.domain_id,
    ...
    if (
        ...
        or len({row.mutation_family for row in examples}) < MIN_MUTATION_FAMILIES
    ...
    for domain_id in {row.domain_id for row in examples}:
        domain_rows = [rows[0] for rows in traces.values() if rows[0].domain_id == domain_id]
        if not any(row.error_index is None for row in domain_rows) or not any(
            row.error_index is not None for row in domain_rows
        ):
    ...
    *,
    ...
    for trace_id, rows in traces.items():
        ...
        if expected is None:
            ...
            within_one += int(predicted is not None and abs(predicted - expected) <= 1)
    ...
    for index in range(10):
        ...
        members = (scores_array >= lower) & (
            (scores_array < upper) | ((index == 9) & (scores_array == 1.0))
        )
        if members.any():
            ece += float(members.mean()) * abs(
                float(scores_array[members].mean()) - float(labels_array[members].mean())
            )
    return {
        "trace_count": count,
    ...
    *,
    ...
    return {
        domain_id: _trace_metrics(
            {trace_id: rows for trace_id, rows in traces.items() if rows[0].domain_id == domain_id},
            probabilities,
            threshold=threshold,
        )
...
def _wilson_bound(successes: int, total: int, *, upper: bool) -> float:
    ...
    bound = (center + margin if upper else center - margin) / denominator
    return round(max(0.0, min(1.0, bound)), 10)


def _process_cell_metrics(
    rows: Sequence[MistakeTransitionExample],
    probabilities: Sequence[float],
    *,
    threshold: float,
) -> dict[str, Any]:
    if len(rows) != len(probabilities):
        raise ValueError("process calibration coverage differs")
    labels = [row.is_error for row in rows]
    predictions = [float(value) >= threshold for value in probabilities]
    correct = sum(predicted is label for predicted, label in zip(predictions, labels, strict=True))
    false_passes = sum(
        predicted and not label for predicted, label in zip(predictions, labels, strict=True)
    )
    ...
    for index in range(10):
        ...
        members = [
            ...
            if lower <= float(probability) < upper or (index == 9 and float(probability) == 1.0)
        ]
        if members:
            mean_probability = sum(float(probabilities[offset]) for offset in members) / len(
                members
            )
            mean_label = sum(float(labels[offset]) for offset in members) / len(members)
            ece += len(members) / len(labels) * abs(mean_probability - mean_label)
    scores = np.asarray(probabilities, dtype=np.float64)
    label_values = np.asarray(labels, dtype=np.float64)
    accuracy_lb = _wilson_bound(correct, len(labels), upper=False)
    false_pass_ub = _wilson_bound(false_passes, negatives, upper=True)
    admitted = (
        len(labels) >= MIN_PROCESS_CELL_EXAMPLES
        and positives >= MIN_PROCESS_CELL_CLASS_EXAMPLES
        and negatives >= MIN_PROCESS_CELL_CLASS_EXAMPLES
        and accuracy_lb >= 0.50
        and false_pass_ub <= 0.50
        and _auc(scores, label_values) >= 0.65
        and brier <= 0.22
        and ece <= 0.20
    )
    return {
        ...
        "accuracy": round(correct / max(1, len(labels)), 10),
        "accuracy_wilson_lower": accuracy_lb,
        "false_pass_wilson_upper": false_pass_ub,
        "transition_auc": round(_auc(scores, label_values), 10),
        "transition_brier": round(brier, 10),
        "transition_ece": round(ece, 10),
        "admitted": admitted,
    }
...
def _process_calibration(
    ...
    *,
    ...
    probability_by_example: dict[str, float] = {}
    for trace_id, trace_rows in _group_traces(examples).items():
        values = list(probabilities[trace_id])
        if len(values) != len(trace_rows):
            raise ValueError("process trace probability coverage differs")
        probability_by_example.update(
            {row.example_id: float(value) for row, value in zip(trace_rows, values, strict=True)}
        )
    result: dict[str, dict[str, Any]] = {}
    for domain_id in sorted({row.domain_id for row in examples}):
        result[domain_id] = {}
        for depth_band in PROCESS_DEPTH_BANDS:
            rows = [
                row
                for row in examples
                if row.domain_id == domain_id
                and process_depth_band(row.transition_index, row.transition_count) == depth_band
            ]
            if not rows:
                continue
            feature_distances = [
                float(
                    np.sqrt(
                        np.mean(
                            ((np.asarray(row.features, dtype=np.float64) - means) / scales) ** 2
                        )
                    )
                )
                for row in rows
            ]
            result[domain_id][depth_band] = {
                "relation": relation,
                "feature_distance_max": round(max(feature_distances), 10),
                "feature_shift_alpha": round(1.0 / (len(feature_distances) + 1), 10),
                **_process_cell_metrics(
                    rows,
                    [probability_by_example[row.example_id] for row in rows],
                    threshold=threshold,
                ),
            }
    return result
...
class MistakeLocatorHead:
    ...
    def probability(
        ...
        return float(
            _sigmoid(np.asarray([self.logits(prior_hidden, candidate_hidden) / self.temperature]))[
                0
            ]
        )
    ...
    def load(
        ...
        *,
        ...
        try:
            ...
            if not stat.S_ISREG(before.st_mode) or not 1 <= before.st_size <= MAX_ARTIFACT_BYTES:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or any(not _is_sha256(manifest[name]) for name in fields if name.endswith("_sha256"))
            or any(
                type(manifest[name]) is not int or not 1 <= manifest[name] <= 100_000
            ...
            != 3
            or manifest["train_domains_sha256"] != manifest["in_domain_domains_sha256"]
        ...
        if is_process_calibrated:
            if manifest["process_calibration_schema"] != PROCESS_CALIBRATION_SCHEMA:
                raise ValueError("process calibration schema is invalid")
            calibration = manifest["process_calibration"]
            if not isinstance(calibration, Mapping) or set(calibration) != {
                "in_domain",
                "out_of_domain",
            }:
                raise ValueError("process calibration relations are invalid")
            process_metric_fields = {
                "relation",
                "sample_count",
                "positive_count",
                "negative_count",
            ...
            for relation, expected_domains in (
                ("in_domain", manifest["in_domain_by_domain"]),
                ("out_of_domain", manifest["out_of_domain_by_domain"]),
            ):
                domain_cells = calibration[relation]
                if not isinstance(domain_cells, Mapping) or set(domain_cells) != set(
                    expected_domains
                ):
                    raise ValueError("process calibration domain coverage differs")
                for _domain_id, depth_cells in domain_cells.items():
                    if not isinstance(depth_cells, Mapping) or not set(depth_cells) <= set(
                        PROCESS_DEPTH_BANDS
                    ):
                        raise ValueError("process calibration depth coverage differs")
                    for cell in depth_cells.values():
                        if (
                            not isinstance(cell, Mapping)
                            or set(cell) != process_metric_fields
                            or cell["relation"] != relation
                            or type(cell["admitted"]) is not bool
                            or any(
                                type(cell[name]) is not int or cell[name] < 0
                                for name in (
                                    "sample_count",
                                    "positive_count",
                                    "negative_count",
                            ...
                            or cell["positive_count"] + cell["negative_count"]
                            != cell["sample_count"]
                            or not 0 <= cell["correct_count"] <= cell["sample_count"]
                            or not 0 <= cell["false_pass_count"] <= cell["negative_count"]
                            or not math.isfinite(float(cell["feature_distance_max"]))
                            or float(cell["feature_distance_max"]) < 0.0
                            or cell["feature_shift_alpha"]
                            != round(1.0 / (cell["sample_count"] + 1), 10)
                            or any(
                                not math.isfinite(float(cell[name]))
                                or not 0.0 <= float(cell[name]) <= 1.0
                                for name in process_metric_fields
                                - {
                                    "relation",
                                    "sample_count",
                                    "positive_count",
                            ...
                            raise ValueError("process calibration cell is invalid")
                        expected_admission = (
                            cell["sample_count"] >= MIN_PROCESS_CELL_EXAMPLES
                            and cell["positive_count"] >= MIN_PROCESS_CELL_CLASS_EXAMPLES
                            and cell["negative_count"] >= MIN_PROCESS_CELL_CLASS_EXAMPLES
                            and cell["accuracy_wilson_lower"] >= 0.50
                            and cell["false_pass_wilson_upper"] <= 0.50
                            and cell["transition_auc"] >= 0.65
                            and cell["transition_brier"] <= 0.22
                            and cell["transition_ece"] <= 0.20
                        )
                        if cell["admitted"] is not expected_admission:
                            raise ValueError("process calibration admission differs")
        if (
        ...
        if manifest["failure_reasons"] != failures or manifest["admitted"] is not (not failures):
    ...
    def fit(
        ...
        *,
        ...
        receipt_sets = [{row.trace_receipt_sha256 for row in split} for split in splits]
        if any(
            left & right
            for collection in (
                example_sets,
        ...
        output_weights = rng.normal(0.0, 1.0 / math.sqrt(hidden_width), size=hidden_width)
        output_bias = 0.0
        ...
        for step in range(steps):
            ...
            gradient = (_sigmoid(logits) - y_train) * sample_weights / len(y_train)
            output_gradient = hidden.T @ gradient + 1e-4 * output_weights
            hidden_gradient = gradient[:, None] * output_weights[None, :] * (1.0 - hidden * hidden)
        ...
        def score(
            ...
            for row in sorted(examples, key=lambda item: (item.trace_id, item.transition_index)):
                vector = np.asarray(row.features, dtype=np.float64)
                hidden = np.tanh(((vector - means) / scales) @ input_weights + input_bias)
        ...
        temperature = float(
            min(
                ...
                key=lambda candidate: float(
                    np.mean(
                        [
                            (float(_sigmoid(np.asarray([logit / float(candidate)]))[0]) - label)
                            ** 2
                            for trace_id, logits in in_logits.items()
                            for logit, label in zip(logits, in_labels[trace_id], strict=True)
                        ]
                    )
                ),
            )
        ...
        def probabilities(
            ...
            return {
                trace_id: [
                    float(_sigmoid(np.asarray([value / temperature]))[0]) for value in values
                ]
        ...
        in_metrics = _trace_metrics(in_domain_traces, in_probabilities, threshold=threshold)
        out_metrics = _trace_metrics(out_domain_traces, out_probabilities, threshold=threshold)
        ...
        process_calibration = {
            "in_domain": _process_calibration(
                in_domain,
                in_probabilities,
                relation="in_domain",
                threshold=threshold,
            ...
            "out_of_domain": _process_calibration(
                out_of_domain,
                out_probabilities,
                relation="out_of_domain",
                threshold=threshold,
        ...
        manifest = {
            ...
            "out_of_domain_domains_sha256": _identity(out_of_domain, "domain_id"),
            ...
            "process_calibration_schema": PROCESS_CALIBRATION_SCHEMA,
            "process_calibration": process_calibration,
