...
def test_masked_copy_write_only_changes_publicly_writable_registers() -> None:
    controller = _controller()
    values = (0, 7, 11, 13, 0)
    state = controller.exact_probabilities(
        values,
...
def test_semantic_write_authority_tracks_selected_balance_and_case_reset() -> None:
    ...
    def authority(case: int, name: int) -> tuple[bool, ...]:
        action = controller.exact_probabilities(
            (11, case, name, 4, 32, 32, 32, 0),
            slots=controller.config.action_slots,
            cardinality=controller.config.action_cardinality,
        )
        mask = controller.transition_write_authority_mask(state, action)
        mx.eval(mask)
