...
| **4. Affect Steering** | `not proven` | **Downgraded 2026-08-06 from `locally demonstrated`.** The supporting tests exercise affect-state coupling and prompt modulation — they show the state is computed and plumbed. They do not show the CONTENT of the state changes generation, and while they passed, the injection path was silently degrading to unsteered output: `affective_steering.py` called `mx.norm`, which does not exist in this MLX version, from a diagnostic running every 50th injection AFTER the steering had been applied. The AttributeError reached the enclosing handler, which discarded the block and returned the original unsteered hidden state. Fixed (symbol corrected, and the diagnostic can no longer void the injection), but the fix means every prior steering measurement ran against a path that was throwing away injections on a fixed schedule. `tools/affect_causality_ablation.py` is the replacement test and asks the sharper question — real state versus a magnitude-matched PERMUTATION of the same vector, which isolates content from magnitude. It has not yet returned a positive result; see 4a |
...
| 4 | Affect steering | Structured affect vector modulates generation (measurable A/B difference) | `core/phases/affect_update.py`, `core/consciousness/affective_steering.py` | `pytest tests/test_affect_behavioral.py tests/test_affective_steering_runtime_hardening.py -q` | blocked | Steering hook inert (d~0 in A/B) -> behavioural affect tests fail. That was the falsifier and it did not fire: the tests passed while the injection path threw away steering on every 50th block via a diagnostic calling a non-existent mx.norm, because the tests check that affect is COMPUTED and PLUMBED, not that its content reaches generation. The replacement falsifier is sharper: real state must beat a magnitude-matched permutation of its own vector by more than the measured noise floor, and the harness withholds the verdict when the metric never fires or the treatment never injects |
