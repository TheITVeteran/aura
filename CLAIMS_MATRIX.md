...
| **4a. Shipped Steering Magnitude Is Below Its Own Effect Threshold** | `not proven` | **Measured 2026-08-06, and it is a defect rather than a claim.** Claim 4 establishes that affect content causally shapes output at injection magnitude 150. The shipped configuration does not run at that magnitude: `DEFAULT_ALPHA` is 5.0, `_INJECTION_ALPHA_CEILING` clips it to 3.0, and `_effective_alpha()` derates to `_STALE_SAFE_ALPHA` 0.35 whenever the substrate sync is more than 2s stale — which is every generation longer than two seconds. Measured on a 1.5B: at magnitude 1 and at 3 the steered output is **byte-identical** to unsteered; the first change appears near 10 and text degenerates by 30. On the 32B the usable window is roughly 10-300 with degeneration by 1000. So the mechanism is live, wired, and configured below the level at which it does anything to greedy-decoded text. Nothing in the runtime notices, because the injection genuinely happens — `_inject_count` advances — it is simply too small to change a token. Closing this needs the magnitude expressed relative to the residual-stream norm (so one setting is meaningful across model sizes) plus a coherence budget measured per model, not a hand-picked constant |
...
| 4a | Shipped steering magnitude is below its own effect threshold | The magnitude the runtime actually injects is too small to change greedy-decoded output | measured: at magnitude 1 and 3 the steered output is byte-identical to unsteered (1.5B); shipped effective alpha is 3.0, derating to 0.35 on any generation over 2s | `python tools/affect_causality_ablation.py --responder mlx --model <id> --alpha 3` versus `--alpha 150` | blocked | Closed when a run at the SHIPPED configuration reproduces claim 4's result — output tracking the held state at p<0.05 while a magnitude-matched permutation does not. Until then the mechanism is wired and set below the level at which it does anything, and nothing in the runtime notices because the injection genuinely occurs (`_inject_count` advances) and is merely too small to move a token. Raising the constant alone is not a fix: the magnitude has to be relative to the residual-stream norm, or one setting means different things on a 1.5B and a 32B |
