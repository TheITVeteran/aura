...
ARMS: tuple[Arm, ...] = (
    ...
    # The same complete engine with one answer-blind candidate generated by the
    # qualified recurrent controller inside the model session. It is separate
    # from the uncomposed treatment so the learned tissue's marginal effect is
    # measured rather than hidden inside a changed arm definition.
    Arm(
...
def _implementation_manifest() -> dict[str, str]:
    ...
    paths = (
        ...
        REPO_ROOT / "tools/rlc_integrated_recurrent_producer.py",
    ...
    body = json.dumps(
        {
            ...
            "integrated_recurrent_max_tokens": int(integrated_recurrent_max_tokens),
...
def _integrated_recurrent_progress_callback(
    out_dir: Path,
    *,
    arm: str,
    task_id: str,
    ...
    def report() -> None:
        ...
        if state["tokens"] % 8 == 0:
            _status(
                out_dir,
                phase="integrated_recurrent_decode",
                arm=arm,
                task_id=task_id,
...
def _manifest_integrity_issues(
    ...
    resource_target = recorded.get(
        "resource_dominating_target_arm",
        "complete_system_closed_book",
    )
    if (
        resource_target
        ...
        or (
            RESOURCE_DOMINATING_CONTROL_ARM in required_set
            and resource_target not in required_set
        )
    ):
        issues.append("resource_dominating_target_arm_invalid")
    recurrent_package = recorded.get("integrated_recurrent_package")
    recurrent_max_tokens = recorded.get("integrated_recurrent_max_tokens")
    if "complete_system_recurrent_composed" in required_set:
        if (
            not isinstance(recurrent_package, dict)
            or set(recurrent_package)
            != {
                "package_id",
                "manifest_sha256",
                "controller_sha256",
            ...
            or not isinstance(recurrent_package.get("package_id"), str)
            or not recurrent_package["package_id"]
            or any(
                not isinstance(recurrent_package.get(name), str)
                or len(recurrent_package[name]) != 64
                or any(
                    character not in "0123456789abcdef"
                    for character in recurrent_package[name]
                )
                for name in (
                    "manifest_sha256",
                    "controller_sha256",
            ...
            issues.append("integrated_recurrent_package_identity_invalid")
        if (
            type(recurrent_max_tokens) is not int
            or not 16 <= recurrent_max_tokens <= 2048
        ):
            issues.append("integrated_recurrent_budget_invalid")
...
def _integrated_candidates_for_profile(
    *,
    profile: str,
    loaded: Any,
    model: Any,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Qualified recurrent shadow package used only by the explicit "
            "complete_system_recurrent_composed arm."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Terminal decode ceiling for the answer-blind recurrent producer.",
    )
    ...
    resource_dominating_target_arm = (
        "complete_system_recurrent_composed"
        if any(arm.name == "complete_system_recurrent_composed" for arm in selected)
        else "complete_system_closed_book"
    )
    ...
    if any(arm.name in RECURRENT_COMPOSED_ARMS for arm in selected):
        if not 16 <= args.integrated_recurrent_max_tokens <= 2048:
            ...
            return 2
        if not args.integrated_recurrent_package:
            print(
                "composed recurrent arm requires --integrated-recurrent-package",
                file=sys.stderr,
            )
            return 2
        ...
        integrated_recurrent_package = Path(
            args.integrated_recurrent_package
        ).expanduser().resolve(strict=True)
        verified_package = inspect_shadow_package(integrated_recurrent_package)
        package_manifest = verified_package["manifest"]
        qualified_activation = read_qualified_activation()
        if (
            qualified_activation.get("mode") != "qualified_typed_only"
            or qualified_activation.get("serving_authority") is not True
            or qualified_activation.get("package_id")
            != package_manifest.get("package_id")
            or qualified_activation.get("manifest_sha256")
            != package_manifest.get("manifest_sha256")
        ):
            print(
                "composed recurrent package lacks matching qualified activation",
                file=sys.stderr,
            )
            return 2
        integrated_recurrent_identity = {
            "package_id": str(package_manifest["package_id"]),
            "manifest_sha256": str(package_manifest["manifest_sha256"]),
            "controller_sha256": str(qualified_activation["controller_sha256"]),
            "activation_sha256": str(qualified_activation["activation_sha256"]),
        }
    ...
    with (
        ...
        if integrated_recurrent_package is not None:
            ...
            if any(
                loaded_integrated_recurrent.receipt.get(name)
                != integrated_recurrent_identity.get(name)
            ...
            if (
                ...
                return 2
            print(
                "integrated recurrent package loaded: "
                f"{integrated_recurrent_identity['package_id']}",
                flush=True,
            )
        ...
        for cell in journal.cells():
            ...
            if task is None:
                continue
            if cell.get("arm") == resource_dominating_target_arm:
                from core.brain.llm.latent_cortex.resource_accounting import (
                    validate_information_receipt,
                    validate_resource_receipt,
                )
...
def grade(out_dir: Path, tasks) -> dict[str, Any]:
    ...
    resource_dominating_target_arm = str(
        (recorded or {}).get("resource_dominating_target_arm")
        or "complete_system_closed_book"
    )
    ...
    if resource_dominating_control_measured:
        ...
        for task_id in sorted(dominance_task_ids):
            treatment_cell = unique_cells.get(
                (resource_dominating_target_arm, task_id)
            )
    ...
    verdict = {
        ...
        "control_contracts": {
            ...
            RESOURCE_DOMINATING_CONTROL_ARM: {
                ...
                "target_arm": resource_dominating_target_arm,
                "resource_matched": False,
                "resource_advantaged": resource_target_control_proven,
                "claim_authority": (
                    "conservative_architectural_gain_control"
                    if resource_target_control_proven
                    else "none"
                ),
            },
        },
