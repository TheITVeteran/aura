...
MATERIALIZATION_INTENT_SCHEMA = (
    "aura.verified_transition.launch_materialization_intent.v2"
)
MATERIALIZATION_RECEIPT_SCHEMA = (
    "aura.verified_transition.launch_materialization_receipt.v2"
)
...
def _strict_json(
    ...
    *,
    ...
    if (
        ...
        or raw not in {canonical_json_bytes(document), canonical_json_bytes(document) + b"\n"}
    ):
...
def _build_signer(
    ...
    return CommandRoleSignerBroker(
        ...
        inherited_environment_names=document.get(
            "inherited_environment_names"
        ),
    )
...
def _validate_signer_role_separation(
    ...
    if (
        ...
        or task_broker.custody_evidence_sha256
        == verifier_broker.custody_evidence_sha256
    ):
...
def _validate_preregistration_envelope(
    ...
    if (
        ...
        or not isinstance(
            contract["paths"].get("verified_launch_bundle"), str
        )
    ):
...
def _recover_completed_launch(
    *,
    ...
    observed_receipt_sha256 = unsigned_receipt.pop(
        "receipt_sha256", None
    )
    ...
    expected_intent_body = {
        ...
        "preregistration_contract_sha256": contract[
            "contract_sha256"
        ],
        ...
        "task_issuer_signer_config_sha256": _digest(
            task_signer_document
        ),
        "evidence_verifier_signer_config_sha256": _digest(
            verifier_signer_document
        ),
    ...
    if (
        ...
        or receipt.get("materialization_intent_sha256")
        != expected_intent["intent_sha256"]
    ):
    ...
    for role, signer_input in signer_inputs.items():
        ...
        expected_fields = {
            key: signer_input[key]
            for key in _SIGNER_CONFIG_KEYS
            if key != "schema"
        }
        if any(
            archived_signer.get(key) != value
    ...
    if (
        ...
        != contract["contract_sha256"]
        or receipt.get("initial_policy_probe_sha256")
        != probe["receipt_sha256"]
        or receipt.get("provider_contract_sha256")
        != provider_contract["contract_sha256"]
        or receipt.get("trust_policy_sha256")
        != archive.trust_policy.policy_sha256
        or receipt.get("campaign_schedule_root_sha256")
        != provider_contract["campaign_schedule_root_sha256"]
        or receipt.get("task_count")
        != len(provider_contract["task_schedule"])
        or receipt.get("bundle_sha256")
        != archive.external_bundle_sha256
    ):
        _fail("materialization_receipt_reconstruction_mismatch")
    if (
        probe["campaign_id"] != contract["campaign_id"]
        or probe["initial_policy_sha256"]
        != provider_contract["initial_policy_sha256"]
        or probe["dataset_sha256"]
        != provider_contract["dataset_sha256"]
        or probe["tokenizer_bundle"]
        != provider_contract["tokenizer_bundle"]
    ):
...
def _task_material(
    *,
    ...
    for sequence, task in enumerate(train):
        nonce = (
            answer_nonces[task.task_id]
            if answer_nonces is not None
...
def materialize_launch(
    *,
    ...
    task_signer_document = _load_signer_config(
        task_issuer_signer_config_path
    )
    verifier_signer_document = _load_signer_config(
        evidence_verifier_signer_config_path
    )
    ...
    for role, broker in brokers.items():
        ...
        if (
            ...
            or pin["custody_evidence_sha256"]
            != broker.custody_evidence_sha256
        ):
    ...
    provider_contract = build_verified_transition_provider_contract(
        ...
        scorer_source_sha256=callable_source_sha256(
            score_verified_recurrent_training_task
        ),
        token_codec_identity=TOKEN_CODEC_ID,
        token_encoder_source_sha256=callable_source_sha256(
            recurrent_trace_token_encoder
        ),
        token_decoder_source_sha256=callable_source_sha256(
            recurrent_trace_token_decoder
        ),
    ...
    signer_bundles = {
        ...
        | {
            ...
            "custody_evidence_sha256": (
                task_broker.custody_evidence_sha256
            ),
        },
        "evidence_verifier": {
            key: verifier_signer_document[key]
            for key in _SIGNER_CONFIG_KEYS
            if key != "schema"
        }
        | {
    ...
    unsigned_bundle = {
        ...
        "preregistration_contract": _binding(
            published["preregistration-contract.json"]
        ),
        ...
        "task_answer_nonces": _binding(
            published["task-answer-nonces.json"]
        ),
    }
    ...
    if (
        reopened.contract_sha256 != provider_contract["contract_sha256"]
    ...
    receipt_body = {
        ...
        "trust_policy_sha256": policy.policy_sha256,
        "campaign_schedule_root_sha256": provider_contract[
            "campaign_schedule_root_sha256"
        ],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        receipt = materialize_launch(
            ...
            evidence_verifier_signer_config_path=(
                args.evidence_verifier_signer_config
            ),
        )
    except (
