...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--families", type=_csv_strings, default=("khop", "boolean", "modular"))
...
def _run(args: argparse.Namespace, *, model_lane_lease: object) -> int:
    ...
    from core.brain.llm.latent_cortex.experiments import TASK_FAMILIES, task_battery
    ...
    sources = {
        ...
        "task_generator": _source_binding(
            REPO_ROOT / "core/brain/llm/latent_cortex/experiments.py"
        ),
    }
    ...
    try:
        while step < args.max_steps and time.monotonic() < deadline:
            ...
            if step % args.log_every == 0:
                ...
                loss_trail.append(
                    {"step": step, "mean_loss": round(mean_loss, 6)}
                )
                window_losses.clear()
                print(
    ...
    from core.brain.llm.latent_cortex.adapter_identity import inspect_mlx_tensor_metadata
