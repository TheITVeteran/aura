...
from tools.reqproof.schema import CLOSED_STATES, Registry, Requirement
        ...
        if not target.is_file():
            defects.append(
                Defect(
                    ...
                    subject=subject,
                    detail="evidence ref does not exist",
                )
            )
            continue
...
def validate_registry(
    ...
    *,
    ...
    for requirement in registry.requirements:
        defects.extend(_verify_evidence(requirement, root, commit_exists))
        ...
        if requirement.state == "complete":
            ...
            verified = _verified_evidence_classes(requirement, root, commit_exists)
            missing = [
